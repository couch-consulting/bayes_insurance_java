    ...
    public void itemStateChanged(ItemEvent ie) {
        ...
        if ((obj instanceof String) &&
            ...
            try {
                ...
                if (s.equals("Auto Select")) np.setNodeStyle(selectedNodes, NodePanel.NODE_STYLE_AUTO_SELECT);
                else if (s.equals("Belief Bars")) np.setNodeStyle(selectedNodes, NodePanel.NODE_STYLE_BELIEF_BARS);
                else if (s.equals("Labeled Box")) np.setNodeStyle(selectedNodes, NodePanel.NODE_STYLE_LABELED_BOX);
                else if (s.equals("Circle")) np.setNodeStyle(selectedNodes, NodePanel.NODE_STYLE_CIRCLE);
                else if (s.equals("Text")) np.setNodeStyle(selectedNodes, NodePanel.NODE_STYLE_TEXT);
                else if (s.equals("Hidden")) np.setNodeStyle(selectedNodes, NodePanel.NODE_STYLE_ABSENT);
                else
...
class LinkChooser extends JPanel implements ItemListener {
    ...
    public void itemStateChanged(ItemEvent ie) {
        ...
        if ((obj instanceof String) &&
                (ie.getStateChange() == ItemEvent.SELECTED)) {
            String s = (String) obj;
            if (s.equals("None")) np.setLinkPolicy(NetPanel.LINK_POLICY_NONE);
            else if (s.equals("Below")) np.setLinkPolicy(NetPanel.LINK_POLICY_BELOW);
            else if (s.equals("Above")) np.setLinkPolicy(NetPanel.LINK_POLICY_ABOVE);
            else
        ...
        try {
            ...
            if (comp instanceof NodePanel) {
                ...
                if (me.paramString().indexOf("button=1") >= 0) { //-- left button was clicked
                    ...
                    } else if (np.getDisplayMode() == NodePanel.DISPLAY_MODE_HILITED) {
                        np.setDisplayMode(NodePanel.DISPLAY_MODE_NORMAL);
                    }
                } else if ((me.paramString().indexOf("button=2") >= 0) ||
